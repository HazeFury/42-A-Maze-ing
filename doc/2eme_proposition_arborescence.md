...
Tous ceux que je suis allée voir (Quentin D, Adrien, Vincent, Léo et Quentin, Lucia, Paul) ont intégré au package le solver. (le sujet demande bien ´access at least a solution' p 13)
	PARSER
3 groupes avaiemt intégré le parser
...
Mettre un exemple de config valid dans le README interne au package.


Question de la classe MazeGenerator:

Dans le cluster du haut, on a souvent séparé les responsabilités :
	-generateur dans le MazeGenerator
	-solver séparé
arguments: clarté, séparation des responsabilités

Dans le cluster du bas, on lit "You must implement the maze generation as a unique class (e.g., ‘MazeGenerator‘) inside
a standalone module that can be imported in a future project." comme une injonction à tout mettre dans la class MazeGenerator: le générateur, le solver et même pour le groupe d'Adrien et pour celui de Vincent Le parser. 
arguments : respect consigne, longueur moyenne de la classe(-300 lignes)
...
arguments : une seule classe comme demande le sujet et un code propre

A partir de là j'amende t proposition de structure :
